...
export default function ModifyBaggageForm(props){
    ...
    if(props.force!= null){
      fetch('/api/forceclosebaggage', requestOptions)
      ...
      fetch('/api/modifybaggage', requestOptions)
    .then(response => response.json())
    .then(data => 
        {
            if(data.status == "success"){
    ...
    for(let i = 0;i<props.data.length;i++){
        if(props.data[i]['name'] == props.baggage){
            idx = i;
            break;
        }
    }
    return (
      ...
      <LocalizationProvider dateAdapter={AdapterDayjs}>
        ...
        >
          ...
          {(props.force!==null)?"Emergency closure":"Modify Baggage"} <b>{props.data[idx]['name']}</b>
          </Typography>
