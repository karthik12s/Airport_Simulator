...
cluster=MongoClient("mongodb+srv/?retryWrites=true&w=majority")
...
app = Flask(__name__)
app.secret_key = "abc"
@app.route("/api/airportsignup",methods = ['get','post'])
def airportsignup():
...
@app.route("/api/airlinesignup",methods = ['get','post'])
def airlinesignup():
...
@app.route("/api/airportlogin",methods = ['get','post'])
def airportlogin():
	collection = db['AirportEmployee']
	if request.method == "POST":
		d = request.json
		...
		if entry == None:
			return {"status":"fail","message":"User doesnot exist"}
		if entry['password']!=d['password']:
			return {"status":"fail","message":"Enter correct password"}
		if entry['approved'] != 1:
			return {"status":"fail","message":"Your request has not been approved"}
		session['username'] = d['email']
		session['name'] = entry['name']
		session['org'] = 'airport'
		return {"status":"success","message":"Success","name":entry['name']}
	return {"status":"fail","message":"Page cannot be accessed"}

@app.route("/api/airlinelogin",methods = ['get','post'])
def airlinelogin():
	collection = db['AirlineEmployee']
	if request.method == "POST":
		d = request.json
...
@app.route("/api/approverequests",methods = ['get','post'])
def approveRequests():
	if "username" in session and session['org'] == 'airport':
		...
		if request.method == "POST":
			...
			if pending_user == None:
				return {"status":"fail","message":"No such user exists"}
			...
			return {"status":"success","message":"User Has been Approved successfully"}
		airport_pending = []
		airline_pending = []
	...
	return {"status":"fail","message":"You cannot access this page"}

@app.route("/api/getairlines")
def getairlines():
...
@app.route("/api/insertairline",methods = ['get','post'])
def insert_airline():
	if "username" in session and session['org'] == 'airport':
		a = getairlines()
		if request.method == "POST":
			...
			if collection.find_one({'name':data['name']})==None:
				...
				return {"message":"Airline has been inserted succesfully","status":"success"}
			return {"message":"The Airline is already present in the list","status":"success"}
		return a
	return {"message":"You cannot access this page"}

@app.route("/api/insertgate",methods = ['get','post'])
def insertgate():
	if "username" in session and session['org'] == 'airport':
		if request.method == "POST":
			d = request.json
...
def getflights():
	...
	threehr = [0,0,0]
	d = {'A':1,'D':2}
	for i in flights_collection.find({"time":{"$lt":datetime.datetime.timestamp(datetime.datetime.now(tz=timezone('US/Pacific')))+3600,'$gt':datetime.datetime.timestamp(datetime.datetime.now(tz=timezone('US/Pacific')))-3600}}).sort('time'):
		del i['_id']
		i['time'] ="-".join(str(datetime.datetime.fromtimestamp(i['time'],tz=timezone('US/Pacific'))).split("-")[:-1])
